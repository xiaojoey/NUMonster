...
app.post('/upload', function(req, res) {
    ...
    if (req.body.pdbId) {
        ...
        makeXMLFile(file, url_path, email);
    }
});
...
function makeXMLFile(file, url_path, email){
    let index = Math.random().toString(36).slice(-10);
    let dirxml = './test/jobs/' + index;
    fs.mkdirSync(dirxml);
    filexml = dirxml + '/' + index + '.xml';
    fs.writeFile(filexml, makeXML(file, url_path, index, email), (err) => {
        if (err) throw err;
        console.log('The file has been saved!');
    });
}

function makeXML(file, url_path, index, email) {
    let chain_pair = [];
    let selected_pairs = [];
    let txt = `<ChainPairs index='${index}' protons='false'><File>${url_path}</File><Email>${email}</Email><IP>Unknown</IP><OS>Unknown</OS>`;
    for (let chain_pair of selected_pairs) {
        txt += `<ChainPair index='${chain_pair}'>${getChainXML(chain_pair[0])}${getChainXML(chain_pair[1])}</ChainPair>`;
    }
    txt += '</ChainPairs>';
    console.log(txt);
    return txt;
