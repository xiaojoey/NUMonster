...
int find_atom(int caindex, char chnid, int atmtyp, int atomnum)
{
    ...
    if (debug) printf("Inside find_atom\n");
	...
	return (atomnum);
    for(i=(atomnum<30)?0:atomnum-30; i < atomnum+30 && i < natoms; i++)
...
int find_atom2(int caindex, char chnid,char * atmnam, int atomnum)
{
	...
	if (atom[i].caindex == caindex && !strncmp(atom[i].atmnam+2,atmnam+2,2) && atom[i].chnid == chnid
      ...
      if(debug) printf("no return on star!\n");
    }

    /*then detect series of protons that belong to branched Ns with the same branch number as the C*/
    /*N4 in Adenine*/
    /*N6 in Cysteine*/
    /*N2 in Guanine*/
    if(atmnam[3] == ' ' && (atmnam[0] == '1' || atmnam[0] == '2') && (atmnam[2] == '2' || atmnam[2] == '4' || atmnam[2] == '6')){
      if (atom[atomnum].caindex == caindex && !strncmp(atom[atomnum].atmnam+2,atmnam+2,2) && atom[atomnum].chnid == chnid
	  && atom[atomnum].atmnam[1] == 'N'){
	if(debug) printf("passed first time %s\n", atom[atomnum].atmnam);
	return (atomnum);
      }
	...
	if (atom[i].caindex == caindex && !strncmp(atom[i].atmnam+2,atmnam+2,2) && atom[i].chnid == chnid
	    && atom[i].atmnam[1] == 'N'){
	  if(debug) printf("passed within 30 %s\n", atom[i].atmnam);
	  return (i);
	}
      }
	...
	if (atom[i].caindex == caindex && !strncmp(atom[i].atmnam+2,atmnam+2,2) && atom[i].chnid == chnid
	    && atom[i].atmnam[1] == 'N'){
	  if(debug) printf("passed within all %s\n", atom[i].atmnam);
	  return (i);
	}
      if(debug) printf("no return on branched protons!\n");
    } 

    /*then detect RNA-unique proton*/
    char temp[] = "*HO2";
    if(!strncmp(temp,atmnam,4)){
      char temp2[] = " O2*";
      /*star detected*/ 
      if(debug) printf("*HO2 detected\n");

      if (atom[atomnum].caindex == caindex && !strncmp(atom[atomnum].atmnam,temp2,3) && atom[atomnum].chnid == chnid){
	if(debug) printf("passed!\n");
	return (atomnum);
      }
      
      for(i=(atomnum<30)?0:atomnum-30; i < atomnum+30 && i < natoms; i++){
      if (atom[i].caindex == caindex && !strncmp(atom[i].atmnam,temp2,3) && atom[i].chnid == chnid){
	  if(debug) printf("passed!\n");
	  return (i);
	}
      }
	...
	if (atom[i].caindex == caindex && !strncmp(atom[i].atmnam,temp2,3) && atom[i].chnid == chnid){
      ...
      if(debug) printf("no return on RNA-unique!\n");
    }

    /*if: same chain/residue, and remoteness and branch same, then means same heavy atom*/
    if (atom[atomnum].caindex == caindex && !strncmp(atom[atomnum].atmnam+2,atmnam+2,2) && atom[atomnum].chnid == chnid)
	return (atomnum);

    /*else iterate from 30 back (or zero) through to this atom to find heavy*/
    /*debug=2;*/
